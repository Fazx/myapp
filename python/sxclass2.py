...
class ShanXi():
    ...
    def getoken(self):
        res = self.session.get("https://zrrwt.sx-n-tax.gov.cn:8082/portals/web/loginForDzswj", headers=self.headers,
                               verify=False)
        html = res.text
    ...
    def login(self, cardtype,name, mm, code):
        headers = {
            "Content-Type": "application/json;charset=UTF-8",
            "User-Agent": "Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.132 Safari/537.36"
        }
        data = {"yhm": "", "idType": self.cardtypes[cardtype], "idNumber": name, "mm": mm, "authCode": code, "redirect_uri": "",
            ...
            # res=self.session.post("https://zrrwt.jl-n-tax.gov.cn:10803/wsz-ww-web/web/wszm/getPzkjzl",headers=self.headers).json()
                res = self.session.get(
                    "https://zrrwt.sx-n-tax.gov.cn:8082/wsz-ww-web/web/wszm/mxcxForGdshb?skssqq=" + str(
                        i) + "-01-01&skssqz=" + str(
                        i) + "-12-31&kjfwSwjg=21400000000&pzkjzl=Z99001002&_=" + self.nowtime,
                    headers=self.headers).json()
                if res["data"]["skmxs"]:
...
if __name__ == "__main__":
    ...
    uname = sx.login(cardtype,name, mm, code)
    if uname:
        sx.detail(uname)
    else:
        pass
