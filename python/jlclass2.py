...
class Jilin():
    ...
    def getmm(self,password,pubkey):
        ...
        return cipher_text
    def login(self,cardtype,card,mm,code):
        headers={
            "Content-Type": "application/json;charset=UTF-8",
            "User-Agent": "Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.132 Safari/537.36"
        }
        data={"yhm":"","idType":self.cardtypes[cardtype],"idNumber":card,"mm":mm,"authCode":code,"redirect_uri":"","response_type":"","client_id":"","sign":"","st":"","dllx":"yhm"}
    ...
    def detail(self,uname):
        try:
            for i in range(2006,self.atime+1):
                ...
                resp=self.session.get("https://zrrwt.jl-n-tax.gov.cn:10803/wsz-ww-web/web/base/code/list/DM_GY_SRLYSWJG",headers=self.headers)
    ...
    try:
        ...
        while True:
            ...
            if not data:
                ...
                uname=jl.login(cardtype,card,mm,code)
                if uname:
                    jl.detail(uname)
                    break
                else:
