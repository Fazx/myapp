...
class FaGui():
    ...
    def getpage(self):
        ...
        res=self.session.get("http://www.chinatax.gov.cn/n810341/n810755/index.html",headers=self.headers)
        html=etree.HTML(res.text)
        urls=html.xpath('.//span[@id="comp_3849171"]/dl/dd/a/@href')
    ...
    def detail(self,htmls,url):
        ...
        title=html.xpath('.//li[@class="sv_texth1_red"]/text()')[0]

        content=re.search('<li class="sv_texth3" id="tax_content">(.*?)</li>',htmls,re.S).group(1)
        date=re.search('\d+年\d+月\d+日',htmls).group().replace("年","-").replace("月","-").replace("日","")
