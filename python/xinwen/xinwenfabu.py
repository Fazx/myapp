...
import pymysql
import logging
logging.basicConfig(level = logging.INFO,format = '%(asctime)s - %(name)s - %(levelname)s - %(message)s',filename="/home/wwwlogs/python/guojiayaowen.txt")
...
class YaoWen():
    ...
    def getpage(self):
        res = self.session.get("http://www.chinatax.gov.cn/n810219/n1865308/index.html", headers=self.headers)
    ...
    def detail(self, htmls, url):
        ...
        date = re.search('<div class="zuo1">(.*?)&', htmls, re.S).group(1).replace("年", "-").replace("月",
                                                                                                     "-").replace(
            "日", "")
...
       uid：文章id
        ...
        item["type"] = pymysql.escape_string('[\"35\"]')
        item["status"] = 2
        item["need_login"] = 0
        item["date"] = date
        item["uid"] = uid
        ...
        item["type_list"]=pymysql.escape_string('[\"3\",\"44\"]')
