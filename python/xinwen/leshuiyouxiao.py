...
import logging
logging.basicConfig(level = logging.INFO,format = '%(asctime)s - %(name)s - %(levelname)s - %(message)s',filename="/home/wwwlogs/python/leshuiyouxiao.txt")
...
class LeShui():
    ...
    cityid={
        "全文有效":pymysql.escape_string('[\"2\",\"7\",\"14\",\"19\"]'),
        "条款失效":pymysql.escape_string('[\"2\",\"7\",\"14\",\"20\"]'),
        "全文废止":pymysql.escape_string('[\"2\",\"7\",\"14\",\"21\"]'),
        "全文失效":pymysql.escape_string('[\"2\",\"7\",\"14\",\"22\"]')
            }
    ...
    def getindexs(self,res):
        for index,i in enumerate(res["validityNodes"]):
            print(i)

    def getpage(self,res):

        for j in res["validityNodes"]:
            item = {}
            city=j["name"]
            ...
            # item["city"]=city
            # item["area"]=area
            #https://www.leshui365.com/law/v1/?s10-new.html
            res=self.session.get("https://www.leshui365.com/law/v"+id+"/?s10-new.html",headers=self.header,verify=False)
            ...
            for url in urls:
                ...
                try:
                    ...
                    detail_item["type_name"]="法规库,全部法规,有效性,"+city
                    #print(detail_item)
                    Db = Dbmysql()
    ...
    def detail(self,res):
        try:
            ...
            item["column_list_id"] = pymysql.escape_string('[\"96\"]')
            item["need_login"] = 0
            item["date"] = date
            item["uid"] = uid
